...
/**
 * A utility to serialize and deserialize Npc objects to/from our custom Config
 * files.
...
class NpcSerializer {
    ...
    /**
     * The definitive deserializer. It reclaims existing NPCs from any Citizens
     * registry
    ...
    public static Npc deserialize(Config config, UUID flameId) {
        ...
        if (config.getRaw().contains("entity-type")) {
            try {
                type = EntityType.valueOf(config.getRaw().getString("entity-type"));
