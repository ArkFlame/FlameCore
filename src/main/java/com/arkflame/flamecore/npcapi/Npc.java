...
public class Npc {
    ...
    // --- Setters & Getters ---
    public void setInitialSpawnLocation(Location location) {
    ...
    public Location getLocation() {
        return isSpawned() ? citizensNpc.getEntity().getLocation() : citizensNpc.getStoredLocation();
    }
    ...
    public UUID getCitizensId() {
        return citizensNpc.getUniqueId();
    }

    public void setNoDamageTicks(int ticks) {
        this.noDamageTicks = Math.max(0, ticks);
    }
    ...
    public void setHitFrequency(int ticks) {
        this.hitFrequency = Math.max(1, ticks);
    }
    ...
    public void setCustomDamage(double damage) {
    ...
    public void spawn() {
        if (initialSpawnLocation == null) {
            NpcAPI.getPlugin().getLogger()
                    .warning("Attempted to spawn NPC '" + getName() + "' without an initial location set.");
            return;
        }
    ...
    public void teleport(Location location) {
        CitizensCompat.teleport(citizensNpc, location);
    }
    ...
    public void save() {
        ...
        if (this.initialSpawnLocation == null) {
            NpcAPI.getPlugin().getLogger().warning("Attempted to save persistent NPC '" + getName()
                    + "' without an initial spawn location. Aborting save.");
            return;
        }
    ...
    public void addAlly(LivingEntity entity) {
        if (entity != null)
            this.allies.add(entity.getUniqueId());
    }

    public void removeAlly(LivingEntity entity) {
        if (entity != null)
            this.allies.remove(entity.getUniqueId());
    }
    ...
    public boolean isAlly(Entity entity) {
        return allies.contains(entity.getUniqueId());
    }
    ...
    public void setEquipment(EquipmentSlot slot, ItemStack item) {
        if (!isSpawned() || getHandle().getEntity() == null) {
            NpcAPI.getPlugin().getLogger()
                    .warning("Attempted to set equipment on a despawned NPC '" + getName() + "'.");
            return;
        }
        ...
        switch (slot) {
            ...
            case OFF_HAND:
                try {
                    ...
                    NpcAPI.getPlugin().getLogger().warning("Cannot set off-hand item for NPC '" + getName()
                            + "'. This server version does not support it.");
                } catch (Exception e) {
                    // Any other exception is a real error during invocation.
                    NpcAPI.getPlugin().getLogger()
                            .severe("Failed to set off-hand item for NPC '" + getName() + "' via reflection.");
                    e.printStackTrace();
                }
                break;
        }
