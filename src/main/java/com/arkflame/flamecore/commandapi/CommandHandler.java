...
public class CommandHandler implements CommandExecutor, TabCompleter {
    ...
    public void register(Command command) {
        ...
        if (pluginCommand == null) {
            plugin.getLogger().severe("Could not register command '" + command.getName()
                    + "'. Did you forget to add it to your plugin.yml?");
            return;
        }
    ...
    @Override
    public boolean onCommand(CommandSender sender, org.bukkit.command.Command bukkitCommand, String label,
            String[] args) {
    ...
    private LinkedList<String> processQuotedArgs(String[] args) {
        ...
        // Safety check for unclosed quotes, though it's unlikely with how commands are
        // parsed.
        if (inQuotes) {
            processed.add(quotedArg.toString());
        }
    ...
    private void executeCommand(CommandSender sender, Command command, LinkedList<String> args)
        ...
        if (args.size() < requiredArgsCount || args.size() > expectedArgs.size()) {
            // --- THE DEFINITIVE FIX: Handle custom vs. auto-generated usage ---

            String customUsage = command.getUsage();
            if (customUsage != null) {
                // If a custom usage string is set, send ONLY that string.
                // We use our ColorAPI here as it's a core utility for formatting.
                sender.sendMessage(ColorAPI.colorize(customUsage).toLegacyText());
            } else {
                // If no custom usage is set, fall back to the auto-generated default.
                String autoGeneratedArgs = expectedArgs.stream()
                        .map(arg -> arg.isRequired() ? "<" + arg.getName() + ">" : "[" + arg.getName() + "]")
                        .collect(Collectors.joining(" "));

                sender.sendMessage(ChatColor.RED + "Wrong usage! Correct format:");
                sender.sendMessage(ChatColor.YELLOW + "Usage: /" + command.getName() + " " + autoGeneratedArgs);
            }
            ...
            if (i < args.size()) {
                ...
                if (parser == null) {
                    throw new CommandException(
                            "Internal error: No parser found for type " + expectedArg.getType().getSimpleName());
                }
                try {
                    Object parsed = parser.parse(rawArg, (Class) expectedArg.getType());
                    parsedArgs.put(expectedArg.getName(), parsed);
                } catch (ArgumentParseException e) {
                    // Prepend the argument name to the error for clarity (e.g., "target: Player not
                    // found")
        ...
        if (command.getExecutor() != null) {
            ...
            if (!command.getSubCommands().isEmpty()) {
                ...
                // This case is unlikely but possible if a command is registered with no
                // executor.
    ...
    @Override
    public List<String> onTabComplete(CommandSender sender, org.bukkit.command.Command bukkitCommand, String alias,
            String[] args) {
        Command command = commands.get(bukkitCommand.getName().toLowerCase());
        if (command == null)
    ...
    private List<String> getCompletions(CommandSender sender, Command command, LinkedList<String> args) {
        ...
        if (argIndex >= 0 && argIndex < expectedArgs.size()) {
            ...
            if (parser != null) {
                List<String> argCompletions = parser.getTabCompletions(sender, currentInput,
                        (Class) expectedArg.getType());
